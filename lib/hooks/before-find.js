        ...
        if (options && options.where && i18n_model) {
            for (var prop in options.where) {
                ...
                if (Array.isArray(options.where[prop])) {
                    ...
                    // Look for the model in includes
                    options.include.forEach(function (incl) {
                        // patched this line because incl.model is apparently not present anymore in sequelize v4 (at least for n:m relations)
                        if (incl.model && incl.model.name && incl.model.name === i18n_model.name) {
                            incl.where = incl.where || {};
                            incl.where[prop] = options.where[prop];
                        }/* else {
                            if (incl.association && incl.association.name && incl.assocation.name === i18n_model.name) {
                                incl.where = incl.where || {};
                                incl.where[prop] = options.where[prop];
                            }
                        } */
